...
static void create_tree_nodes (Context *ctx) {
  ...
  for(int i = 0; i < NUMBER_OF_CHARS; i++) {
     if(ctx->table[i].v > 0) {
       n = tree_new();
       n->type = LEAF;
       n->freq.v = ctx->table[i].v;
       n->freq.c = ctx->table[i].c;
...
static TreeNode *build_tree (Context *ctx) {
  ...
  while(pqueue_size(ctx->pq) > 1) {
     l = pqueue_dequeue(ctx->pq);
     r = pqueue_dequeue(ctx->pq);
     sum = l->freq.v + r->freq.v;
     n = tree_new();
     n->type = INTERNAL;
     n->freq.v = sum;
     //printf("Sum -> %d\n", sum);
