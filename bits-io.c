...
//Method used to get the most significant
//bit
int get_most_sig(int byte) {
  int param = byte;
  int bit;
  for(int i = 0; i < 8; i++) {
    bit = param % 2;
...
int bits_io_read_bit (BitsIOFile *bfile) {
  ...
  // times, shift in a 0.  (We found it easy to have a variable indicating the
  ...
  if(bfile->byte != ALL_BITS_READ) { 
     ...
     if(bfile->byte == ALL_BITS_READ) {
       ...
       if(get_most_sig(bfile->byte) == 0) {
         bfile->byte = ((bfile->byte << 1)|1);
         //printf("I Got Here -> %d\n",get_most_sig(bfile->byte));
         return (get_most_sig(bfile->byte));
       }
       bfile->byte = ((bfile->byte << 1)|1);
       return (get_most_sig(bfile->byte));
     }
     ...
     bfile->byte = ((bfile->byte << 1)|1);
     while(get_most_sig(bfile->byte)!=0) {
        //printf("Hi!");
        bfile->byte <<= 1;
     }
     //printf("\n");
